...
class WorkTypeSqlQueries(SqlQueries):
    def best_score(self) -> str:
        return f"""
            SELECT i.work_type AS work_type, MAX(g.score) AS best_score
    ...
    def mean_score(self) -> str:
        return f"""
            SELECT i.work_type AS work_type, AVG(g.score) as mean_score
            {self._from_game_join_user_survey}
                AND g.score > 100
            GROUP BY i.work_type
        """
    ...
    def time_to_click(self) -> str:
        return f"""
            SELECT i.work_type AS work_type, s.time_to_click AS time_to_click
            FROM game g
    ...
    def best_score_with_game_count(self) -> str:
        return f"""
            SELECT i.work_type AS work_type, MAX(g.score) AS best_score, COUNT(g.score) AS game_count
            {self._from_game_join_user_survey}
                AND g.score > 100
            GROUP BY i.work_type
        """
    ...
    def count_of_games_to_best_score(self) -> str:
        return f"""
            SELECT i.work_type AS work_type, (
                SELECT COUNT(g2.id)
                FROM game g2
    ...
    def score_with_class(self) -> str:
        return f"""
            SELECT g.score AS score, i.work_type AS work_type
            {self._from_game_join_user_survey}
                AND g.score > 100
        """
